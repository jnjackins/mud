...
func Interpolate(env Env, args []string, template string) (string, error) {
	...
	expr, err := NewParser(template).Parse()
	if err != nil {
		return "", err
	}
	return expr.Expand(env, args)
}
...
type Expansion interface {
	Expand(env Env, args []string) (string, error)
	Identifiers() []string
}
...
func (e VariableExpansion) Expand(env Env, args []string) (string, error) {
...
func (e ParameterExpansion) Identifiers() []string {
	return []string{}
}

func (e ParameterExpansion) Expand(env Env, args []string) (string, error) {
	if e.N >= len(args) {
		return "", nil
	}
	return args[e.N], nil
...
func (e StarExpansion) Identifiers() []string {
	return []string{}
}

func (e StarExpansion) Expand(env Env, args []string) (string, error) {
	if len(args) == 0 {
		return "", nil
	}
...
func (e EmptyValueExpansion) Expand(env Env, args []string) (string, error) {
	val, _ := env.Get(e.Identifier)
	if val == "" {
		return e.Content.Expand(env, args)
	}
	return val, nil
}
...
func (e UnsetValueExpansion) Expand(env Env, args []string) (string, error) {
...
func (e SubstringExpansion) Expand(env Env, args []string) (string, error) {
	val, _ := env.Get(e.Identifier)
...
func (e RequiredExpansion) Expand(env Env, args []string) (string, error) {
	val, ok := env.Get(e.Identifier)
	if !ok {
		msg, err := e.Message.Expand(env, args)
		if err != nil {
			return "", err
		}
		if msg == "" {
...
func (e Expression) Expand(env Env, args []string) (string, error) {
	buf := &bytes.Buffer{}

	for _, item := range e {
		if item.Expansion != nil {
			result, err := item.Expansion.Expand(env, args)
			if err != nil {
				return "", err
			}
