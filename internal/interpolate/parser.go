...
func (p *Parser) parseExpansion() (Expansion, error) {
	...
	// if we have * or a number, this is a parameter expansion
	if c == '*' {
		p.nextRune()
		return StarExpansion{}, nil
	} else if '0' <= c && c <= '9' {
		...
		return ParameterExpansion{N: n}, nil
	}
...
func (p *Parser) scanNumber() (string, error) {
	notNumber := func(r rune) bool {
		return r < '0' || r > '9'
	}
	return p.scanUntil(notNumber), nil
}
